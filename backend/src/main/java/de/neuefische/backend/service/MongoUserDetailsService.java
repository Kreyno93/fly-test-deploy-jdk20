...
public class MongoUserDetailsService implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        ...
        return new User(mongoUser.getUsername(), mongoUser.getPassword(), mongoUser.getRoles());
    }

    public MongoUserDTO saveUser(MongoUserDTO user) {
        if (repo.findMongoUserByUsername(user.getUsername()).equals(user.getUsername())){
            throw new IllegalArgumentException("Username already taken");
        }
        PasswordEncoder encoder = Argon2PasswordEncoder.defaultsForSpringSecurity_v5_8();
        MongoUser x = new MongoUser(UUID.randomUUID().toString(), user.getUsername(), encoder.encode(user.getPassword()),Collections.singletonList(
                new SimpleGrantedAuthority("user")));
        repo.save(x);
        return new MongoUserDTO(x.getId(), x.getUsername());
    }
}
